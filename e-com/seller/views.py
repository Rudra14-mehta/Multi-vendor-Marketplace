...
def dashboard(request):
    ...
    for order in seller.order:
        # price = order["price"]
        price = order["products"][0]["price"]
        total_sales += price
        total_orders += 1
    print(f"{total_sales} - {total_orders} - {total_products}")
    if request.method == "POST":
...
def products(request):
    ...
    if request.method == "POST":
        # currProduct = Product.objects.get(id = request.POST['edit'])
        
        # Instance is passed because it is for updating values of product
        form = ProductForm(request.POST, request.FILES)  # include FILES for image
        if "edit" in form.data:
            form.instance = Product.objects.get(id = form.data['edit'])

        # if delete from is submitted
        if "delete" in form.data:
            product = Product.objects.get(id = form.data['delete'])
            product.delete()
            return HttpResponseRedirect("products")
        

        # this is to explicitly pass seller to forms 
...
def orders(request):
    ...
    if request.POST:
        newStatus = request.POST["status_change"]
        id = request.POST["id"]
        print(f"ID = {id}")
        if newStatus:
            # Change Seller order
            for index, order in enumerate(seller.order):
                # print(f"order['orderID']: {type(order["orderID"])}  ---  ID: {type(id)}")
                if str(order["orderID"]) == id:
                    seller.order[index]["status"] = newStatus
                    # print(f" \n\n seller.order[0][order]['status'] -> {seller.order[index]["status"]}")
                    seller.save()
                    break

            # change Orders table
            order = Orders.objects.get(id = id)
            if newStatus.lower() == "processing":
                order.status = "pr"
            else:
                order.status = newStatus[0].lower()
    ...
    for s in seller.order:
        print(f"Order -> {s} \n\n")
    context.update({"orders": seller.order})
    # print(seller.order)
    return render(request, "dashboard/orders.html", context)
